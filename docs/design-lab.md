...
| 4. Put them in scenes | `studio.html`, the Scenes section: **built**, the room from a brief or the pools |
...
- **Nothing the simplified designer could already have made.** Enforced by
  building every plain run with Simple's own algorithm and rejecting anything
  whose identity matches one. `buildPlainRun` is a port of `app.js`'s
  `buildSimpleDesign`. A hand-written "looks like a plain run" predicate would drift the
...
small, complete, deliberate gesture. A design is two or three of them
...
- **Depth 1300.** An L-shape occupies its second axis too, and a turn measures
  ...
  than by judgement.
- **Width 3600.** A real design from the builder measures 3266mm: two towers,
  an adapter bridging their inner posts, boosters staggering the storeys
  above. At 3000 the generator could not have produced it even in principle.
...
roles. Then it grows a design through the builder's own placement engine,
...
which is the inverse of the `shareHash` beside it, so the link format has one reader
...
The planner is deterministic about all of it, same angles and same ids, so the
...
**When the batch runs out, the page grows another.** A corpus is finite, which
...
the normal case, not the edge case but **every** scene, is a finished, paid-for
...
Matched pairs, same shot and same room, the prompt the only change:
...
A five-step, 275 cm, three-footprint shelf with two bridges and a lamp
...
the catalogue is two colours, `marine` being steel `#143F68` carrying MDF
...
and then restates the collars and the square corners at the end, where they are
read last.
...
**Read the two-tone row with care.** The cohorts are not matched, the second
being different shelves at different angles in different rooms, and an earlier
...
JPEG quality, then fewer pixels. It is a fallback, not the path. An earlier
...
At the open end of a run, never the inside of a corner, where a person standing
