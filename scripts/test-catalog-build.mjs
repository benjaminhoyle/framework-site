 ...
 *
 * The first test is the one that matters most: rebuilding the real, live
 * shelving.html from the real catalog.json must reproduce it byte for byte.
 */
...
import {
  CATALOG, FEED, PAGE, buildPublish, catalogJson, feedCsv, imagesOf,
  readCatalog, readConfigurations, requiredAssets, spliceConfigurations,
...
test('rebuilding the live page from the live catalog changes nothing', () => {
  const built = buildPublish({ catalogText, pageText });
  assert.equal(built.files[PAGE], pageText, 'shelving.html would have changed');
...
test('inactive products are kept in catalog.json but never shipped', () => {
  ...
  const before = buildPublish({ catalogText, pageText });
  const off = { ...cat.products[0], id: 'switched-off', active: false };
  const built = buildPublish({ catalogText, pageText, products: [...cat.products, off] });

  assert.ok(!built.files[PAGE].includes('"switched-off"'), 'an inactive product reached the page');
...
test('the page and the feed always describe the same products', () => {
  const built = buildPublish({ catalogText, pageText });
  const onPage = readConfigurations(built.files[PAGE]).map((c) => c.id);
  const inFeed = built.files[FEED].trim().split('\n').slice(1).map((row) => row.split(',')[0]);
  assert.deepEqual(inFeed, onPage);
});
...
test('reading the page back cannot execute what is in it', () => {
  // readConfigurations used to run the array through a JS evaluator. It is
  // JSON.stringify output, so it parses -- and parsing cannot run anything that
  // finds its way into a product description.
  const nasty = [{ id: 'x', title: 'a"b', image: 'i.jpg', images: ['i.jpg'], price: 'Ksh 1', priceValue: 1, description: '</script> ok' }];
  const spliced = spliceConfigurations(pageText, nasty);
  const back = readConfigurations(spliced);
  assert.equal(back.length, 1);
  assert.equal(back[0].description, '</script> ok');
  assert.throws(() => readConfigurations('nothing here'), /could not find configurations/);
});
...
test('a designCode survives a publish', () => {
  ...
  // …and stays out of the page and feed, which have no use for it.
  const built = buildPublish({ catalogText, pageText, products: withCode });
  assert.ok(!built.files[PAGE].includes('1Y3MK7P'));
...
test('a page whose array cannot be found is refused rather than mangled', () => {
  assert.throws(() => spliceConfigurations('<html>no array</html>', []), /not found/);
});
