#!/usr/bin/env node
/**
 * Bump the /new-designer asset version.
 *
...
const PAGE = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "new-designer.html");
const source = fs.readFileSync(PAGE, "utf8");

const declaration = /(window\.frameworkDesignerVersion = ')(\d+)(')/;
const found = source.match(declaration);
if (!found) throw new Error("could not find window.frameworkDesignerVersion in new-designer.html");
...
console.log(`/new-designer assets are now v${next}`);
